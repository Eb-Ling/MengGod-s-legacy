...
public class LightWingEffect2 implements MyRenderTool.MyRender, MyRenderTool.InitializableEffect {
    ...
    private static final String FRAG_SRC =
        ...
        "\n" +
        "    float taperedDist = distFromCenter;\n" +
        "\n" +
        "    float shapeMask = 1.0 - smoothstep(0.65, 1.0, taperedDist);\n" +
        "    shapeMask *= 1.0 - smoothstep(0.82, 1.0, ux);\n" +
        "    shapeMask *= smoothstep(0.0, 0.06, ux);\n" +
        "\n" +
        "    if (shapeMask < 0.01) discard;\n" +
        "\n" +
        "    float t = u_time * u_flowSpeed;\n" +
        "\n" +
        "    float n1 = fbm(vec2(ux * 4.0 - t * 0.7, vy * 3.0));\n" +
        "    float n2 = fbm(vec2(ux * 7.0 - t * 1.2 + 4.0, vy * 4.0 + 1.5));\n" +
        "    float n3 = fbm(vec2(ux * 10.0 - t * 1.8 + 8.0, vy * 5.0 - 1.0));\n" +
        "\n" +
        "    float b1 = smoothstep(0.30, 0.42, n1) * smoothstep(0.72, 0.56, n1);\n" +
        "    float b2 = smoothstep(0.28, 0.40, n2) * smoothstep(0.70, 0.54, n2);\n" +
        "    float b3 = smoothstep(0.32, 0.44, n3) * smoothstep(0.74, 0.58, n3);\n" +
        "\n" +
        "    float layers = b1 * 0.55 + b2 * 0.4 + b3 * 0.25;\n" +
        "\n" +
        "    float dist = abs(vy - 0.5) * 2.0;\n" +
        "    float ripple1 = pow(max(0.0, 1.0 - abs(mod(dist * 5.0 - t * 1.0, 2.5) - 1.25) * 3.5), 2.0);\n" +
        "    float ripple2 = pow(max(0.0, 1.0 - abs(mod(dist * 7.0 - t * 1.4 + 1.0, 2.0) - 1.0) * 4.0), 2.5);\n" +
        "\n" +
        "    float shock = pow(max(0.0, 1.0 - abs(mod(ux * 4.0 - t * 1.2, 3.0) - 1.5) * 2.5), 3.0);\n" +
        "\n" +
        "    float scanline = smoothstep(0.42, 0.5, abs(sin(vy * 55.0))) * 0.12;\n" +
        "\n" +
        "    float energy = 0.06 + layers * 0.35 + (ripple1 * 0.3 + ripple2 * 0.2) + shock * 0.2 + scanline;\n" +
        "\n" +
        "    float edgeGlow = pow(ux, 2.0) * 0.2;\n" +
        "    float rootGlow = pow(1.0 - ux, 3.0) * 0.35;\n" +
        "    float rippleFlash = ripple1 * pow(1.0 - ux, 2.0) * 0.2;\n" +
        "\n" +
        "    vec3 col = mix(u_colorInner, u_colorOuter, ux);\n" +
        "    col += u_colorGlow * (edgeGlow + rootGlow + rippleFlash);\n" +
        "    col *= (0.5 + energy * 1.0);\n" +
        "\n" +
        "    float breath = 0.94 + 0.06 * sin(u_time * 3.5);\n" +
        "    float flicker = 0.97 + 0.03 * sin(u_time * 13.0 + ux * 6.0);\n" +
        "\n" +
    ...
    private void createShaderProgram() {
        try {
            ...
            if (GL20.glGetShaderi(vert, GL20.GL_COMPILE_STATUS) == 0) {
                System.err.println("LightWing2 vert compile failed: " + GL20.glGetShaderInfoLog(vert, 1024));
                return;
            }
            ...
            if (GL20.glGetShaderi(frag, GL20.GL_COMPILE_STATUS) == 0) {
                System.err.println("LightWing2 frag compile failed: " + GL20.glGetShaderInfoLog(frag, 1024));
                return;
            }
            ...
            if (GL20.glGetProgrami(shaderProgram, GL20.GL_LINK_STATUS) == 0) {
                System.err.println("LightWing2 link failed: " + GL20.glGetProgramInfoLog(shaderProgram, 1024));
                return;
            }
            ...
            System.out.println("LightWing2 shader compiled OK");
        } catch (Exception e) {
            System.err.println("LightWing2 shader error: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public void cyclePalette() {
        ...
        System.out.println("LightWing2 palette: " + names[paletteIndex]);
    }
}
