...
public class Meng_fire_sharpnel implements EveryFrameWeaponEffectPlugin {
    private final boolean init = false;

    @Override
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        if (!init) {
            weapon.getDamage().getModifier().modifyMult("Meng_fire_sharpnel", 0.033f);
        }
        if (weapon.getChargeLevel() == 1f) {

            for (int f = 0; f < 29; f++) {

                float offset = ((float) Math.random() * 15) - (15 * 0.5f);
                float angle = weapon.getCurrAngle() + offset;
                Vector2f Location = findlocation(weapon);
                DamagingProjectileAPI proj = (DamagingProjectileAPI) engine.spawnProjectile(weapon.getShip(), weapon, "Meng_fire_sharpnel", Location, angle, weapon.getShip().getVelocity());
                float randomSpeed = (float) Math.random() * 0.6f + 0.6f;
