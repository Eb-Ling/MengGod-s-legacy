 ...
 */
public class Meng_lightoffinner_Effect extends BaseCombatLayeredRenderingPlugin implements OnFireEffectPlugin,
        OnHitEffectPlugin,
        EveryFrameWeaponEffectPlugin {

    public Meng_lightoffinner_Effect() {
    }

    protected List<Meng_lightoffinner_Effect> trails;

    /**
     * 更新武器特效状态，包括冷却管理、拖尾清理、音效播放和弹体位置调整
     *
     * @param amount 时间增量
     * @param engine 战斗引擎实例
     * @param weapon 需要更新特效的武器对象
     */
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        // 辐能低于50时禁用
        if (weapon.getShip().getFluxLevel() <= 0.5f) {
        ...
        // 清理过期的拖尾特效
        Iterator<Meng_lightoffinner_Effect> iter = trails.iterator();
        while (iter.hasNext()) {
        ...
        if (weapon.getShip() != null) {
            ...
            for (Meng_lightoffinner_Effect curr : trails) {
                if (curr.proj != null) {
        ...
        for (int i = 0; i < numIter; i++) {
            for (Meng_lightoffinner_Effect trail : trails) {
                //trail.proj.setFacing(trail.proj.getFacing() + 180f * amount);
                // 拖动前置弹体跟随后续弹体，保持自然连接效果
     ...
     * 处理弹体击中目标时的特效和音效
     *
     * @param projectile   造成伤害的弹体对象
     * @param target       被击中的战斗实体
     * @param point        击中位置坐标
     * @param shieldHit    是否击中护盾
...
//								   rampUp, dur, inverted);
    ...
    public void onFire(DamagingProjectileAPI projectile, WeaponAPI weapon, CombatEngineAPI engine) {
        ...
        Meng_lightoffinner_Effect trail = new Meng_lightoffinner_Effect(projectile, prev);
        ...
        if (trails == null) {
            trails = new ArrayList<Meng_lightoffinner_Effect>();
        }
        trails.add(0, trail);
    }
...
//			offset.y += vel.y * 1f;
    ...
    public Meng_lightoffinner_Effect(DamagingProjectileAPI proj, DamagingProjectileAPI prev) {
        this.proj = proj;
        this.prev = prev;
    ...
    public void render(CombatEngineLayers layer, ViewportAPI viewport) {
        ...
        Color color = new Color(180,155,255,50);
